...
$success = '';
$error = '';
$staffResult = $conn->query("SELECT id, admin_id, username, full_name, created_at FROM admin_users WHERE role = 'staff' ORDER BY id ASC LIMIT 1");
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!hash_equals($_SESSION['staff_account_csrf'], $token)) {
        ...
        if ($duplicate->get_result()->num_rows > 0) {
            ...
            if ($update->execute()) $success = 'Staff account updated successfully.';
            ...
            if ($insert->execute()) $success = 'The staff account was created successfully.';
...
<aside class="sidebar">
    ...
    <div class="sidebar-foot"><div class="avatar"><?php echo htmlspecialchars(strtoupper(substr($_SESSION['full_name'] ?? 'A',0,1))); ?></div><div><strong><?php echo htmlspecialchars($_SESSION['full_name'] ?? 'Admin'); ?></strong><span>Super Admin</span></div><a href="../logout.php" title="Logout"><i class="fas fa-sign-out-alt"></i></a></div>
</aside>
