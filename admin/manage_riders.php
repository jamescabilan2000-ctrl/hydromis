...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'add') {
        ...
        if ($username === '' || $name === '' || $age <= 0 || $address === '' || $phone === '' || $password === '') {
            ...
            $exists = $conn->query("SELECT rider_id FROM rider_users WHERE username = '$username' LIMIT 1");
            if ($exists && $exists->num_rows > 0) {
                ...
                $status = 'active';
                $sql = "INSERT INTO rider_users (rider_id, username, password, full_name, age, address, contact_number, status)
                        VALUES ('$rider_id', '$username', '$hashed_password', '$name', $age, '$address', '$phone', '$status')";
                if ($conn->query($sql)) {
        ...
        if ($rider_id === '' || $username === '' || $name === '' || $age <= 0 || $address === '' || $phone === '') {
            ...
            $exists = $conn->query("SELECT rider_id FROM rider_users WHERE username = '$username' AND rider_id != '$rider_id' LIMIT 1");
            if ($exists && $exists->num_rows > 0) {
                $error = 'Username already exists.';
            } else {
                $sql = "UPDATE rider_users SET username = '$username', full_name = '$name', age = $age, address = '$address', contact_number = '$phone'";
                if ($password !== '') {
                    $hashed_password = password_hash($password, PASSWORD_DEFAULT);
