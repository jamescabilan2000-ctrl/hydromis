...
$seedTransactions = $conn->query("SELECT transaction_id, user_id, amount, payment_status FROM transactions ORDER BY created_at DESC");
if ($seedTransactions) {
...
$payments = $conn->query("SELECT p.payment_id, p.transaction_id, p.transaction_id AS order_id, p.amount, p.payment_status AS status, p.created_at, p.payment_method, p.payment_reference, u.full_name, u.contact_number
    FROM payments p
